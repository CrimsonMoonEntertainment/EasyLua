...
#import <Foundation
#import "CMLuaManager.h"
#import "CMSynthesizeSingleton.h"
#import "ProductDescriber.h"

@implementation CMLuaManager

SYNTHESIZE_SINGLETON_FOR_CLASS(CMLuaManager)
...
-(bool)runFileAtPath:(NSString*)path
{
    if (luaL_dofile(L, [path UTF8String]))
    {
        ...
        CMAlertMessage(@"error while loading file %@: %s",path,  err);
        return false;
    }
...
-(bool)runLuaString:(NSString*)string
{
    ...
    {
        const char *err = lua_tostring(L, -1);
        CMAlertMessage(@"error while loading string %@: %s",string, err);
        return false;
    }

    if (lua_pcall(L, 0, 0, 0)) {
        const char *err = lua_tostring(L, -1);
        CMAlertMessage(@"error while running string %@: %s",string, err);
        return false;
    }
